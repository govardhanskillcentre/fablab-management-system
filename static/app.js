...
/* Firebase Authentication: any Firebase Email/Password user may access the dashboard. */
function logoutUser(){
  if(typeof firebase==='undefined' || !firebase.auth) return;
  firebase.auth().signOut().then(function(){ window.location.replace('/'); }).catch(function(err){ console.error('Logout failed:',err); toast('Logout failed'); });
}
if(typeof firebase!=='undefined' && firebase.auth){
  firebase.auth().onAuthStateChanged(function(user){ if(!user) window.location.replace('/'); });
}
